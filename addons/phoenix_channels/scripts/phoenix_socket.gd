...
const STATUS = {ok = "ok", error = "error", timeout = "timeout"}
...
@export var is_connected := false:
	get = get_is_connected
@export var is_connecting := false:
	get = get_is_connecting
...
func _init(endpoint, opts = {}):
	_settings = {
		heartbeat_interval =
		PhoenixUtils.get_key_or_default(opts, "heartbeat_interval", DEFAULT_HEARTBEAT_INTERVAL_MS),
		timeout = PhoenixUtils.get_key_or_default(opts, "timeout", DEFAULT_TIMEOUT_MS),
		reconnect_after =
		PhoenixUtils.get_key_or_default(opts, "reconnect_after", DEFAULT_RECONNECT_AFTER_MS),
...
func _process(_delta):
	...
	if status != _last_status:
		...
		if status == WebSocketPeer.STATE_CONNECTING:
			...
			if is_connecting:
				emit_signal("on_connecting", false)
			is_connecting = false
	...
	if status == WebSocketPeer.STATE_OPEN:
		...
		if (
			(current_ticks - _last_heartbeat_at >= _settings.heartbeat_interval)
			and (current_ticks - _connected_at >= _settings.heartbeat_interval)
		):
...
func connect_socket():
	...
	_socket.connect_to_url(_endpoint_url, null)
...
func set_endpoint(endpoint: String):
	_settings.endpoint = (
		PhoenixUtils.add_trailing_slash(endpoint if endpoint else DEFAULT_BASE_ENDPOINT) + TRANSPORT
	)
...
func channel(topic: String, params: Dictionary = {}, presence = null) -> PhoenixChannel:
	var new_channel: PhoenixChannel = PhoenixChannel.new(self, topic, params, presence)
...
func _trigger_channel_error(c: PhoenixChannel, payload := {}):
	c.raw_trigger(PhoenixChannel.CHANNEL_EVENTS.error, payload)
	...
	if _should_reconnect:
		...
		if _last_reconnect_try_at == -1:
			...
			if current_time - _last_reconnect_try_at >= reconnect_after:
				...
				if (
					_reconnect_after_pos < reconnect_after - 1
					and _reconnect_after_pos < _settings.reconnect_after.size() - 1
				):
...
func _find_and_remove_channel(c: PhoenixChannel):
	var pos = _channels.find(c)
	if pos != -1:
...
func _on_socket_closed():
	...
	_last_close_reason = (
		{message = "connection lost"} if _last_close_reason.is_empty() else _last_close_reason
	)
	...
	for c in _channels:
		c.close(payload, _should_reconnect)
...
func _on_socket_data_received(packet):
	...
	if json.has("event"):
		...
		if message.get_topic() == TOPIC_PHOENIX:
			...
			for c in _channels:
				if c.is_member(message.get_topic(), message.get_join_ref()):
					c.trigger(message)


func _on_node_removed(node: Node):
	var c = node as PhoenixChannel
	if c:
		_find_and_remove_channel(c)
