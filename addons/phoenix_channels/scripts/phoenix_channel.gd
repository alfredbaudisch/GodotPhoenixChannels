...
const PRESENCE_EVENTS := {diff = "presence_diff", state = "presence_state"}
...
func _init(socket, topic: String, params: Dictionary = {}, presence = null):
	...
	_rejoin_timer.connect("timeout", Callable(self, "_on_Timer_timeout"))
...
func is_member(topic, join_ref) -> bool:
	...
	var is_lifecycle_event = (
		topic == CHANNEL_EVENTS.close
		or topic == CHANNEL_EVENTS.error
		or topic == CHANNEL_EVENTS.join
		or topic == CHANNEL_EVENTS.reply
		or topic == CHANNEL_EVENTS.leave
	)
...
func _rejoin() -> bool:
	if _state == ChannelStates.JOINING or _state == ChannelStates.JOINED:
		...
		if _socket.can_push(CHANNEL_EVENTS.join):
			...
			_join_ref = ref
			_socket.push(
				_socket.compose_message(CHANNEL_EVENTS.join, _params, _topic, ref, _join_ref)
			)
			return true
