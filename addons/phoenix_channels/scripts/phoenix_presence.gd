...
func sync_state(new_state: Dictionary) -> Dictionary:
	...
	return sync_diff({joins = joins, leaves = leaves})
...
func sync_diff(diff: Dictionary) -> Dictionary:
	...
	for key in keys:
		...
		emit_joins.append(
			{key = key, current_presence = current_presence, new_presence = new_presence}
		)
	...
	for key in keys:
		...
		if _state.has(key):
			var current_presence = _state[key]
			var refs_to_remove = PhoenixUtils.map(
				Callable(self, "_get_phx_ref"), left_presence.metas
			)
			current_presence.metas = _find_metas_from_refs(current_presence.metas, refs_to_remove)

			emit_leaves.append(
				{key = key, current_presence = current_presence, left_presence = left_presence}
			)
...
func clear():
	...
	for key in keys:
		var current_presence = _state[key]
		leaves.append(
			{key = key, current_presence = current_presence, left_presence = current_presence}
		)
